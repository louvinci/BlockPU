...
void reduceload_axi( ap_uint<InWidth>  *ddr_burst,
					...
					unsigned short ch,  unsigned row,unsigned col,
...
#pragma HLS array_partition variable=buf dim=1 complete
	...
	DwIN_R:
	for(unsigned char r = 0; r < Tr+6; r++) {
		DwIN_C:
		for(unsigned char c = 0; c < Tc+6; c++) {
...
#pragma HLS ARRAY_PARTITION variable=bn_buff dim=1 factor=32 cyclic
...
#pragma HLS BIND_STORAGE variable=BN_buff type=ram_2p impl=bram
