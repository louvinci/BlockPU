...
int main(){
	...
	//unsigned R1=56,C1=56,N1=32,M1=32*6, K1=3;
    //unsigned R1=112,C1=112,N1=32,M1=32*6, K1=3;
	unsigned R1=28,C1=28,N1=64,M1=64*6, K1=7;
	//unsigned R1=14,C1=14,N1=128,M1=128*4,K1=7;
	//unsigned R1=7,C1=7,N1=256,M1=256*6,K1=7;
	//unsigned R1=14,C1=14,N1=64,M1=64*4, K1=7;
	//unsigned R1=14,C1=14,N1=96,M1=96*4, K1=7;
	unsigned padN1=0;

	if(N1 >DwTn1 && N1 % DwTn1 != 0 ){
		padN1 = N1 + DwTn1 - (N1 % DwTn1);
	}
	unsigned FinalN = (padN1 == 0) ? N1:padN1;
	std::cout<<"Padded N1 is "<<padN1<<std::endl;
	std::cout<<"Final  N1 is "<<FinalN<<std::endl;
	ADtype*   in        =    new ADtype[MXR1*MXC1*MXN1];
    Uint32*   norm_wt   =    new Uint32[NormSize1*2];
	WDtype*   dw_wt     =    new WDtype[MXN1*7*7];
    ...
    QBPU1((ABWtype*)in,(NWtype*)norm_wt,(WBWtype*)dw_wt,(WBWtype2*)pw1_wt,(WBWtype3*)pw2_wt,(OBWtype*)in,(OBWtype*)hw_res,
    		           rescale1,rescale2,id_rescale,R1,C1,M1,N1,K1,padN1);
    ...
    for(int k=0;k<R1;k++){
        for(int i =0;i<C1;i++){
            for(int j=0;j<N1;j++){
            	...
            	tmp2.range(Abit1-1,0) = hw_res[k*C1*FinalN+i*FinalN+j].range(Abit1-1,0);
            	if (tmp!= tmp2){
            		w_cnt+=1;
#ifdef DEBUG
