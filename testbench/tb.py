...
def get_in(H,W,C,datatype,dir,DEBUG=False):
    if datatype == 'int16':
...
def dw_weight_gen(N,K,pack_num,datatype,fdir,DEBUG=False):
    if 'float' in datatype:
        ...
        input = random.randint(-64,64,(N//pack_num,K,K,pack_num)).astype(np.short)
    elif 'int8' in datatype:
        input = random.randint(-64,64,(N//pack_num,K,K,pack_num)).astype(np.int8)
    else:
        raise Exception
...
def get_dw(N,K,pack_num,datatype,dir,DEBUG=False):
    ...
    x = x.reshape(N,K,K) # for the pytorch compute
    if DEBUG:print(x)
    return x
...
def main():
    ...
    input_gen(R1,C1,N1,datatype,os.path.join(dir,"input.bin"),False)
    ...
    input    =   get_in(R1,C1,N1,datatype,os.path.join(dir,"input.bin"),False)
    ...
    with torch.no_grad():
        ...
        q_x = torch.clamp(q_x,-range_out,range_out-1)
